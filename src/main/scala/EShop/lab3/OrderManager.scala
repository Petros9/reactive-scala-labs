...
class OrderManager {
  ...
  def uninitialized: Behavior[OrderManager.Command] = Behaviors.receive[Command] {
    (context, message) =>
      message match {
        case AddItem(id, sender) =>
          val cartActor: ActorRef[TypedCartActor.Command] = context.spawn(new TypedCartActor().start, "cartActor")
  ...
  def open(cartActor: ActorRef[TypedCartActor.Command]): Behavior[OrderManager.Command] = Behaviors.receive[Command] {
    (context, message) =>
      message match {
        case AddItem(id, sender) =>
          cartActor ! TypedCartActor.AddItem(id)
  ...
  def inCheckout(checkoutActorRef: ActorRef[TypedCheckout.Command]): Behavior[OrderManager.Command] = Behaviors.receive[Command] {
    (context, message) =>
      message match {
  ...
  def inPayment(senderRef: ActorRef[Ack]): Behavior[OrderManager.Command] = Behaviors.receive[Command] {
    (_, message) =>
      message match {
  ...
  def finished: Behavior[OrderManager.Command] = Behaviors.receive[Command] {
    (_, message) =>
      message match {
        case _ =>
          Behaviors.stopped
