  ...
  def start: Behavior[TypedCheckout.Command] = Behaviors.receive {
    (context, message) =>
      message match {
        case StartCheckout =>
          selectingDelivery(
            context.scheduleOnce(checkoutTimerDuration,
                                 context.self,
                                 ExpireCheckout))
  ...
  def selectingPaymentMethod(
    ...
    case (context,
          SelectPayment(payment,
                        orderManagerCheckoutRef,
                        orderManagerPaymentRef)) =>
      timer.cancel()
      orderManagerCheckoutRef ! PaymentStarted(
        context.spawn(
          new Payment(payment, orderManagerPaymentRef, context.self).start,
          "PaymentActor")
      )
      processingPayment(
